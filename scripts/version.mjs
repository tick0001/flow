...
//
// Appele par `make version V=0.1.1`, apres que la cible a cree la branche.
...
function poserLeLien(journal) {
  ...
  const lien = `[${version}]: https://github.com/tick0001/flow/releases/tag/v${version}\n`;

  return journal.slice(0, premierLien + 1) + lien + journal.slice(premierLien + 1);
}

/**
 * Le manifeste que lit la route de sante, et sur lequel la publication verifie
 * la concordance.
 *
 * C'est **le seul** manifeste du depot qui porte la version du produit : les
 * autres restent a `0.0.0`. Deux endroits ou ecrire un numero sont deux
 * endroits ou l'oublier, et rien ne dirait lequel a raison.
 */
...
function ouvrirLaSection() {
  ...
  const section = [
    ...
    'publication, fichiers de communauté. Ce journal s’adresse à qui exploite Flow&.',
    '-->',
    '',
    '',
  ].join('\n');
