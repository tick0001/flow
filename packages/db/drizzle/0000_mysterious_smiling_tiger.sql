...
-- passeraient sans rien prouver, ce qui est pire que de ne pas les avoir.
-- =============================================================================
CREATE EXTENSION IF NOT EXISTS ltree;
--> statement-breakpoint
CREATE EXTENSION IF NOT EXISTS citext;
--> statement-breakpoint
-- Recherche plein texte dans les journaux d'execution, et rapprochement des
-- messages d'echec pour les regrouper.
...
--
-- Le mot de passe pose ici est public, puisqu'il est dans ce depot. Sa rotation
-- est obligatoire a l'installation, et documentee comme telle.
DO $preambule$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'flow_app') THEN
    CREATE ROLE flow_app LOGIN PASSWORD 'flow_app';
  END IF;
END
$preambule$;
--> statement-breakpoint
...
CREATE TYPE "public"."right_scope" AS ENUM('own', 'entity', 'recursive', 'all');--> statement-breakpoint
...
CREATE TABLE "entity_settings" (
	...
	"concurrent_execution_limit" integer,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
CREATE TABLE "users" (
